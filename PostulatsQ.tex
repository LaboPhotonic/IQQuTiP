...
$\ket{1}$ et $\ket{\psi}=(\ket{0}+\ket{1})/\sqrt{2}$. Le symbole \# introduit 
un 
commentaire.

\begin{lstlisting}
from qutip import *
B=Bloch() # Cree une sphère de Bloch vide
ket0 = basis(2,0) # $\ket{0}$
ket1 = basis(2,1) # $\ket{1}$
B.add\_states(ket0) # Represente $\ket{0}$ sur B
B.add\_states(ket1) # Represente $\ket{1}$ sur B
B.add\_states((ket0+ket1)/sqrt(2)) # Represente $\ket{\psi}$ sur B
B.show() # Permet de visualiser B avec $\ket{0}$, $\ket{1}$ et $\ket{\psi}$.
\end{lstlisting}
