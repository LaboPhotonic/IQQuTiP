...
différentes classes (\textbf{Qobj()) associées aux fonctions \textbf{operator} 
\textbf{state} avoir obtenir des informations sur les opérateurs.}
...
$\mathtt{H} = \frac{2}{\sqrt{2}} (\sigma_x+\sigma_y)$. 

\item En utilisant  la commande \texttt{Q.isherm} appropriée (Qobj attribue), 
vérifier que $\mathtt{H}$ est hermitienne. Évaluer $\mathtt{H}^2$ et le comparer 
à $\mathbb{I}_2$. En déduire que $\mathtt{H}$ est une matrice unitaire.

\item Définir les projecteurs $P_1$ et $P_2$ sur les états propres de 
$\mathtt{H}$. Vérifier les propriétés d'un opérateur projecteur sur $P_1$ 
(i.e., $P_1^{\dag}=P_1$, $P_1^2=P_1$).
\end{enumerate}
